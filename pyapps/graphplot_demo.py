...
if args.database is not None:
    ...
    for act_id in args.ids:
        ...
        x1, y1s = dict2lists(d1, 'dt', [args.coord], args.relative, mirror)
        data.append([x1, y1s, None, [act_id]])
elif args.file is not None:
    names = ['time', 'east', 'north', 'width', 'height', 'code']
    ...
    for act_id in args.ids:
        ...
        x1 = tmp['time']
        if args.relative:
            y1s = [(tmp[args.coord] - tmp[args.coord].iat[0]) * mirror]
        else:
            y1s = [tmp[args.coord] * mirror]
        data.append([x1, y1s, None, [act_id]])
else:
